...
var create = []Query{
	...
	{name: "hits", query: `CREATE TABLE hits (hit_id UUID PRIMARY KEY DEFAULT gen_random_uuid(), url_id UUID REFERENCES urls(url_id) ON DELETE CASCADE, referer VARCHAR(2000), sec_ch_ua VARCHAR(500), sec_ch_ua_mobile VARCHAR(500), sec_ch_ua_platform VARCHAR(500), user_agent VARCHAR(500), ip_address INET, created_at TIMESTAMPTZ)`},
}
